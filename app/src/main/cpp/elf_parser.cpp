...
ElfImage::ElfImage(std::string_view library_name) {
    if (!findLoadedLibraryInfo(library_name)) {
        ...
        LOGI("%.*s is not loaded in this process", (int) library_name.size(), library_name.data());
        return;
    }
...
ElfW(Addr) ElfImage::findSymbolByGnuHash(std::string_view symbol_name, uint32_t gnu_hash) const {
    // A stripped or malformed .gnu.hash can leave either count at zero; both are
    // used as divisors below.
    if (gnu_bloom_filter_ == nullptr || gnu_bloom_size_ == 0 || gnu_nbucket_ == 0) return 0;
...
ElfW(Addr) ElfImage::findSymbolBySysvHash(std::string_view symbol_name, uint32_t sysv_hash) const {
    if (bucket_ == nullptr || nbucket_ == 0) return 0;
